...
const API_BASE_URL = window.location.hostname === 'localhost' ? 'http://localhost:8000' : 'YOUR_RENDER_BACKEND_URL';
...
export default function App() {
  ...
  const handleDownload = () => {
    ...
    if (activeTab === 'markdown' || activeTab === 'preview') {
      ...
      content = JSON.stringify(result.metadata || result.jsonAst, null, 2);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Connect to the new FastAPI backend on Render
      const endpoint = `${API_BASE_URL}/api/v1/convert`;
                  ...
                  <div className="text-blue-400 space-y-2">
                   ...
                   <div className="w-full h-full overflow-auto p-4 font-mono text-sm leading-relaxed">
                     <pre className="whitespace-pre-wrap break-words">
                       {activeTab === 'markdown' ? result.markdown || 'No Markdown output' : JSON.stringify(result.metadata || result.jsonAst, null, 2)}
                     </pre>
                   </div>
                )}
             </div>
